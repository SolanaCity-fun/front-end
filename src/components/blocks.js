...
class blockFactory extends EventEmitter {
				...
				if (hashes.length) {
					...
					if (typeof hashes[0] === "string") {
						...
						hashes.forEach((hash) => {
							tasks.push(
								fetch(
									`${process.env.VUE_APP_REST_API}/static/blocks/${this.ticker}/${hash}?verbose=false`
								)
							);
						});
						let responses;
	...
	generateSolanaBlock() {
		...
		const futureTime = Math.floor(currentTime / 1000) + 1; // 1 second into the future

		return {
			baseFee: Math.floor(Math.random() * (15000000000 - 11000000000) + 11000000000),
			busCapacity: 40000000 + Math.floor(Math.random() * 20000000),
			coin: "SOL",
			gl: 40000000 + Math.floor(Math.random() * 20000000),
			gu: 30000000 + Math.floor(Math.random() * 25000000),
			hash: "SOL" + Math.random().toString(36).substring(2, 15) + Date.now().toString(36),
			height: this.lastSolanaBlock ? this.lastSolanaBlock.height + 1 : 15879022,
			hoursPast: 0,
			inserted: futureTime,
			minerTime: futureTime - 1,
			parentHash: this.lastSolanaBlock ? this.lastSolanaBlock.hash : solanaBlocks[solanaBlocks.length - 1].hash,
			processed: true,
