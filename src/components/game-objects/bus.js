...
const Bus = new Phaser.Class({
	...
	initialize: function Bus(scene) {
		...
		if (this.scene.config.ticker == "ETH") {
			...
			this.backDoorSprite = this.scene.add.image(
				57,
				45 + this.busHeight - 80,
				getSheetKey("door.png"),
				"door.png"
			);
			this.busMiddle = this.scene.add.rectangle(
				0,
...
Bus.prototype.newBus = function (atStop = true) {
	...
	// Changed CELO-TON Block number
	if (this.scene.config.ticker == "CELO") {
		this.text1.setText("#" + 40887112 + busIndex);
	}
	this.setFeeText();
...
Bus.prototype.setFeeText = function () {
	let text2 = this.feeText
		? this.feeText
		: this.lowFee
		? Math.ceil(this.lowFee) + " " + this.scene.config.ticker == "CELO"
			? "TON"
			: this.scene.vue.busFeeTitle
		: "";
	...
	if (this.hasBridgeTransaction) {
		...
		this.movingTween = this.scene.add.tween({
			...
			onComplete: () => {
				...
				this.brake();
				this.doorOpen();
				eventHub.$emit("AlightBridge", {
					myStartX: this.x,
					myStartY: this.y,
					mySide: this.onSide,
					myRightPoint: this.busRightBridgeStartPoint,
					myBridgeTxData: this.bridgTxs,
				});
				setTimeout(() => {
					this.unbrake();
					this.leaveTween();
...
Bus.prototype.boardingY = function () {
	if (this.scene.config.ticker == "ETH") {
		return this.busTopSprite.getTopLeft(null, true).y + this.scene.busDoorFromTop + toRes(35);
	} else {
